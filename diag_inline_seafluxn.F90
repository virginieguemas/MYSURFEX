...
!
REAL, DIMENSION(:), INTENT(IN) :: PCO2      ! atmospheric co2 (kg/m3) 
!
...
IF (S%LHANDLE_SIC) THEN
   DGS%XTS   (:) = (1.0-S%XSIC(:)) * S%XSST(:) + S%XSIC(:)*S%XTICE(:)
   DGS%XTSRAD(:) = PTRAD(:)
ELSE
   DGS%XTS   (:) = S%XSST(:)
   DGS%XTSRAD(:) = PTRAD (:)
...
!
  IF (DGS%N2M>=1) THEN
     IF (S%LHANDLE_SIC) THEN
        !
        DGS%XT2M (:) = DGS%XT2M (:) * (1.0-S%XSIC(:)) + DGS%XT2M_ICE (:) * S%XSIC(:)
        DGS%XQ2M (:) = DGS%XQ2M (:) * (1.0-S%XSIC(:)) + DGS%XQ2M_ICE (:) * S%XSIC(:)
        DGS%XHU2M(:) = DGS%XHU2M(:) * (1.0-S%XSIC(:)) + DGS%XHU2M_ICE(:) * S%XSIC(:)
        !
        DGS%XZON10M(:) = DGS%XZON10M(:) * (1.0-S%XSIC(:)) + DGS%XZON10M_ICE(:) * S%XSIC(:)
        DGS%XMER10M(:) = DGS%XMER10M(:) * (1.0-S%XSIC(:)) + DGS%XMER10M_ICE(:) * S%XSIC(:)
        !
        DGS%XWIND10M_ICE(:) = SQRT(DGS%XZON10M_ICE(:)**2+DGS%XMER10M_ICE(:)**2)
        !
        DGS%XRI    (:) = PRI    (:) * (1.0-S%XSIC(:)) + PRI_ICE(:) * S%XSIC(:)
        DGS%XRI_ICE(:) = PRI_ICE(:)
        !
     ELSE
        DGS%XRI    =PRI
...
IF(DGS%LSURF_BUDGETC)THEN
  CALL DIAG_SURF_BUDGETC_SEA(DGS, &
                             PTSTEP, DGS%XRN, DGS%XH, DGS%XLE, DGS%XLEIC, DGS%XGFLUX,  &
...
IF (DGS%LCOEF) THEN
   IF (S%LHANDLE_SIC) THEN 
      ...
      !
      DGS%XCD(:) = (1.0-S%XSIC(:)) * PCD(:) + S%XSIC(:) * PCD_ICE(:)
...
IF (DGS%LSURF_VARS) THEN
  ...
  !
   IF (S%LHANDLE_SIC) THEN 
      DGS%XQS    (:) = (1.0-S%XSIC(:)) * PQSAT(:) + S%XSIC * PQSAT_ICE(:)
...
!
  CALL DIAG_CPL_ESM_SEA(S, &
                        ...
                        DGS%XSWD,DGS%XSWU,DGS%XGFLUX,DGS%XEVAP,PRAIN,     &
                        PSNOW,PLW,PCO2,S%XTICE,DGS%XH_ICE,DGS%XLE_ICE,    &
                        ZEVAP_ICE,PDIR_SW,PSCA_SW,                        &
                        DGS%XSWU_ICE,DGS%XLWU_ICE                         )
! 
ENDIF
!
...
!
  CALL DIAG_GLT_SEA(S, &
                    DGS%XSWD,DGS%XSWU,DGS%XGFLUX,DGS%XEVAP,PRAIN,PSNOW, &
                    PLW,S%XTICE,DGS%XH_ICE,DGS%XLE_ICE,ZEVAP_ICE,       &
                    PDIR_SW,PSCA_SW,DGS%XSWU_ICE,DGS%XLWU_ICE           )
! 
ENDIF
!
