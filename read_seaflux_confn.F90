...
!!      Modified    11/2016 : R. Séférian: introduce key for ocean carbon flux computation
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
IF (LNAM_READ) THEN
 ...
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CSEA_SFCO2',CSEA_SFCO2,'NONE','VARI')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CCH_DRY_DEP',CCH_DRY_DEP,'      ','WES89 ','NONE  ')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CINTERPOL_SST',CINTERPOL_SST,'LINEAR','UNIF  ','QUADRA','NONE  ','READAY')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CINTERPOL_SSS',CINTERPOL_SSS,'LINEAR','UNIF  ','QUADRA','NONE  ','READAY')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CINTERPOL_SIC',CINTERPOL_SIC,'LINEAR','UNIF  ','NONE  ','READAY')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CINTERPOL_SIT',CINTERPOL_SIT,'LINEAR','UNIF  ','NONE  ','READAY')
 !
 !
 !* close namelist file
 !
