...
IF (DGS%LCOEF) THEN
   ...
   !
   CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,DGS%XCD_SEA(:),IRESP,HCOMMENT=YCOMMENT)
   !
   YRECFM='CHSEA'
   YCOMMENT='X_Y_'//YRECFM//' (W/s)'
   !
   CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,DGS%XCH_SEA(:),IRESP,HCOMMENT=YCOMMENT)
   !
   YRECFM='CESEA'
   YCOMMENT='X_Y_'//YRECFM//' (W/s)'
   !
   CALL WRITE_SURF(DGU, U, &
                  HPROGRAM,YRECFM,DGS%XCE_SEA(:),IRESP,HCOMMENT=YCOMMENT)
   !
   IF (S%LFORMDRAG) THEN
      !
      YRECFM='CD_FORM'
